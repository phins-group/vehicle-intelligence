...
async def test_model_suggestions_overlay_unlabeled_items_without_mutating_source(
    ...
    generated = next(
        ...
        if item.suggestions
        and item.suggestions[0].attributes.get("suggestionRunId")
        == result.suggestion_run_id
    )
...
async def test_promotion_carries_pending_model_suggestion_overlay_forward(
    ...
    original = next(
        ...
        for item in page.items
        if item.suggestions
        and not item.suggestions[0].attributes.get("suggestionRunId")
    )
    ...
    pending = [
        json.loads(line)
        for line in (target / "REVIEW_QUEUE.jsonl").read_text().splitlines()
    ]
...
async def test_review_overlay_is_revisioned_restart_safe_and_filterable(tmp_path: Path) -> None:
    ...
    corrected = await service.review(
        ...
        DetectorReviewCommand(
            ...
            reviewer=_operator(),
            annotations=(
                DetectorReviewAnnotation(DetectorReviewBox(18, 22, 76, 31)),
            ),
    ...
    assert '"status":"PENDING_REVIEW"' in (
        source_root / source_id / "REVIEW_QUEUE.jsonl"
    ).read_text()
...
async def test_review_validates_actions_and_promotes_new_immutable_source(
    ...
    with pytest.raises(DatasetReviewValidationError, match="inside the source image"):
        await service.review(
            ...
            DetectorReviewCommand(
                ...
                reviewer=_operator(),
                annotations=(
                    DetectorReviewAnnotation(DetectorReviewBox(130, 90, 20, 20)),
                ),
            ),
        )
    ...
    await service.review(
        ...
        DetectorReviewCommand(
            ...
            reviewer=_operator(),
            annotations=(
                DetectorReviewAnnotation(DetectorReviewBox(22, 27, 64, 21)),
            ),
    ...
    promoted_evidence = [
        json.loads(line)
        for line in (target / "REVIEW_DECISIONS.jsonl").read_text().splitlines()
    ]
