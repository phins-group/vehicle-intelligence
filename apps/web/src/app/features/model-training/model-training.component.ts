...
export class ModelTrainingComponent implements OnInit, OnDestroy {
  ...
  private logRequestGeneration = 0;
  private runRefreshGeneration = 0;
  ...
  ngOnDestroy(): void {
    ...
    this.logRequestGeneration += 1;
    this.runRefreshGeneration += 1;
    ...
    try {
      ...
      if (selectedRun) {
        ...
        if (
          ...
          isModelTrainingActive(selectedRun.status)
        ) {
          this.schedulePoll(selectedRun.id);
        }
      }
    } catch (error) {
      if (!this.destroyed && generation === this.loadGeneration) {
        this.loadState.fail(
          apiErrorMessage(error, 'Không thể tải quy trình build model.'),
        );
      }
    } finally {
      if (!this.destroyed && generation === this.loadGeneration)
  ...
  async startRun(): Promise<void> {
    const dataset = this.selectedDataset();
    if (this.destroyed || !dataset || !this.canStart()) return;
    ...
    try {
      ...
      this.success.set(
        'Training run đã được ghi audit và đưa vào hàng đợi an toàn.',
      );
      this.schedulePoll(run.id, 700);
    } catch (error) {
      if (!this.destroyed) {
        this.error.set(
          apiErrorMessage(error, 'Không thể bắt đầu training run.'),
        );
      }
    } finally {
      if (!this.destroyed) this.starting.set(false);
    }
  }

  async refreshSelectedRun(): Promise<void> {
    const run = this.selectedRun();
    if (this.destroyed || !run || this.refreshingRun()) return;
    const generation = ++this.runRefreshGeneration;
    this.refreshingRun.set(true);
    try {
      const refreshed = await firstValueFrom(this.api.modelTrainingRun(run.id));
      if (this.destroyed || generation !== this.runRefreshGeneration) return;
      this.upsertRun(refreshed);
      if (this.selectedRunId() !== refreshed.id) return;
      await this.loadLogs(refreshed);
      if (
        !this.destroyed &&
        generation === this.runRefreshGeneration &&
        this.selectedRunId() === refreshed.id &&
        isModelTrainingActive(refreshed.status)
      ) {
        this.schedulePoll(refreshed.id);
      }
    } catch (error) {
      if (!this.destroyed && generation === this.runRefreshGeneration) {
        this.error.set(
          apiErrorMessage(error, 'Không thể cập nhật training run.'),
        );
      }
    } finally {
      if (!this.destroyed && generation === this.runRefreshGeneration) {
  ...
  async cancelSelectedRun(): Promise<void> {
    ...
    if (
      this.destroyed ||
      !run ||
      !isModelTrainingActive(run.status) ||
      this.canceling()
    )
      return;
    if (
      !window.confirm(
        'Hủy GPU training job này? Chi phí đã phát sinh sẽ không được hoàn lại.',
      )
    ) {
      return;
    }
    ...
    try {
      const canceled = await firstValueFrom(
        this.api.cancelModelTrainingRun(run.id),
      );
      if (this.destroyed) return;
      this.upsertRun(canceled);
      this.success.set(
        'Đã gửi yêu cầu hủy và ghi nhận audit cho training run.',
      );
      if (this.pollTimer !== null) window.clearTimeout(this.pollTimer);
    } catch (error) {
      if (!this.destroyed) {
      ...
      if (!this.destroyed) this.canceling.set(false);
    }
  }
  ...
  private initializeForm(
    ...
    this.selectedSourceId.set(
      ready?.sourceId ?? registry.items[0]?.sourceId ?? '',
    );
  ...
  private ensureSelections(): void {
    if (
      !this.datasets().some((item) => item.sourceId === this.selectedSourceId())
    ) {
      this.selectedSourceId.set(this.datasets()[0]?.sourceId ?? '');
    }
    if (!this.runs().some((item) => item.id === this.selectedRunId())) {
      const preferred = this.runs().find((item) =>
        isModelTrainingActive(item.status),
      );
  ...
  private schedulePoll(runId: string, delay = 3500): void {
    ...
    this.pollTimer = window.setTimeout(async () => {
      this.pollTimer = null;
      if (this.destroyed || this.selectedRunId() !== runId) return;
      await this.refreshSelectedRun();
    }, delay);
  }

  private async loadLogs(run: ModelTrainingRun): Promise<void> {
    const generation = ++this.logRequestGeneration;
    if (!run.remoteJobId) {
      if (this.destroyed || this.selectedRunId() !== run.id) return;
      this.logLines.set([]);
      this.logsAvailable.set(false);
      return;
    }
    try {
      const logs = await firstValueFrom(
        this.api.modelTrainingLogs(run.id, 300),
      );
      if (
        this.destroyed ||
        generation !== this.logRequestGeneration ||
      ...
      if (
        ...
        this.logError.set(
          apiErrorMessage(error, 'Log từ remote job chưa khả dụng.'),
        );
      }
    }
  }
